...
export default function CartDrawer() {
  ...
  const displayIsOpen = mounted ? isOpen : false;
  ...
  return (
    <>
      {displayIsOpen && <button className="fixed inset-0 w-full h-full bg-black/40 z-50 transition-opacity cursor-default" onClick={closeCart} aria-label="Close cart" />}

      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 flex flex-col shadow-2xl transition-transform duration-300 ${displayIsOpen ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
              ...
              <div>
                ...
                <p className="text-sm text-stone-400 mt-1">
                  {displayLanguage === "FR" ? "Découvrez nos dernières pièces" : "Discover our latest pieces"}
                </p>
              </div>
              <Link href="/products" onClick={closeCart}
                ...
                {displayLanguage === "FR" ? "Découvrir" : "Shop Now"}
              </Link>
            </div>
          ) : (
          ...
          <div className="border-t border-stone-100 px-6 py-6 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-medium" style={{ fontFamily: "var(--font-cormorant), serif" }}>
                {formatLocalPrice(total() / CAD_RATE, displayCurrency, total())}
              </span>
            </div>
            <p className="text-xs text-stone-400">
              {displayLanguage === "FR" ? "Taxes calculées lors du paiement" : "Taxes calculated at checkout"}
            </p>
